"""
Інтеграційні тести для перевірки взаємодії між модулями підписок та генерації контенту.
"""
...
def app():
    ...
    with app.app_context():
        ...
        # Створюємо тестового користувача з закінченою підпискою
        expired_user = User(
...
def test_premium_subscription_and_trends_access(client):
    """
    Тест для перевірки доступу до трендів для користувача з преміум підпискою.
    """
    # Отримуємо токен для преміум користувача
...
def test_free_subscription_and_trends_access(client):
    """
    Тест для перевірки доступу до трендів для користувача з безкоштовною підпискою.
    """
    # Отримуємо токен для безкоштовного користувача
...
def test_expired_subscription_and_trends_access(client):
    """
    Тест для перевірки доступу до трендів для користувача з закінченою підпискою.
    """
    # Отримуємо токен для користувача з закінченою підпискою
    token_response = client.get('/test-token/expired@example.com')
    token_data = json.loads(token_response.data)
    token = token_data.get('token')
    ...
    subscription_data = json.loads(subscription_response.data)
    assert subscription_data.get('subscription_type') == 'premium'
    assert subscription_data.get('is_active') == False
...
def test_update_subscription_and_trends_access(client):
    """
    Тест для перевірки доступу до трендів після оновлення підписки.
    """
    # Отримуємо токен для безкоштовного користувача
    token_response = client.get('/test-token/free@example.com')
    token_data = json.loads(token_response.data)
    ...
    # Намагаємося отримати тренди (має бути заборонено)
    trends_data = {
        'category': 'Технології'
    }
    ...
    # Тепер намагаємося отримати тренди (має бути дозволено)
    trends_response = client.post('/trends',
                                data=json.dumps(trends_data),
                                headers={'Authorization': f'Bearer {token}'},
                                content_type='application/json')
